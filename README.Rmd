...
inventory — ready to query, plot, map, and export with simple R functions.

## What's Inside

- **Data**: GHG emissions for all 27 Brazilian states (1990–2022), across
  5 IPCC sectors and 7 gas types, from the First Biennial Transparency
  Report (BTR1/MCTI)
...
(MCTI)** via the
[SIRENE platform](https://www.gov.br/mcti/pt-br/acompanhe-o-mcti/cgcl/clima) —
...
[TIC-DAD-02](inst/config/schemas_sqlite.sql), using camelCase with
class prefixes:
...
Table prefixes: `Dim` (dimension), `Fat`/`fact` (fact), `Hist` (history).
